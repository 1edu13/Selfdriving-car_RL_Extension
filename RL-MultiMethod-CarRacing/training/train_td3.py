    ...
    for episode in range(1, args.total_episodes + 1):
        ...
        while True:
            if total_steps < args.warmup_steps:
                ...
                action = agent.select_action(obs, noise=args.exploration_noise)
        ...
        if ep_reward > best_ep_reward:
            ...
            agent.save(os.path.join(args.save_dir, "td3_best.pth"))

        if episode % args.save_freq == 0:
            agent.save(os.path.join(args.save_dir, f"td3_ep{episode}.pth"))
...
def parse_args():
    p = argparse.ArgumentParser(description="Train TD3 on CarRacing-v2")
    ...
    p.add_argument("--save_dir", type=str, default="models/td3_results")
